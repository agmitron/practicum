import { getProjectTasks, addTask, deleteTask } from "./api.js";
...
const createTaskElement = (task) => {
  ...
  deleteButton.addEventListener('click', () => {
    deleteTask(task.id)
      ...
      .catch((err) => {
        console.log(err);
        showError(taskText, `Не удалось удалить карточку: ${err}`, 2000)
      });
...
const setTaskSubmitListener = (tasksContainer, projectId) => {
  ...
  submitForm.addEventListener('submit', (evt) => {
     ...
     addTask(input.value, projectId)
      ...
      .catch((err) => {
        showError(tasksList, `Не удалось добавить карточку: ${err}`, 2000)
      })
...
export const loadProjectTasks = (projectId) => {
  ...
  getProjectTasks(projectId)
    .then((tasks) => renderTasks(tasks.reverse(), projectId))
    .catch((err) => {
      showError(tasksContainer, `Не удалось загрузить список задач: ${err}`)
    })
    .finally(() => {
